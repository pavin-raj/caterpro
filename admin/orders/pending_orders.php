...
$sql = "SELECT * FROM orders o INNER JOIN tbl_user u ON o.user_id = u.user_id AND o_status='Pending'";
...
<body>
  ...
  <main>
          ...
          if(mysqli_num_rows($result)>0)
            {
              while($row=mysqli_fetch_array($result))
              {
                ...
                <input type='button' class='button-secondary' value='Approve' onclick='approveOrder($row[0])'>";
                echo "</div>";
              }
            }
          ?>
    ...
    - custom js link
  -->
      <script src="/caterpro/assets/js/script.js"></script>
